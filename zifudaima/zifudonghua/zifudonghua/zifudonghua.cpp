// zifudonghua.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
	...
	char hi[52800];					//这里是字符数组，用于存储从文件读取出的字符串
	FILE *in;							//定义一个FILE类型的结构体指针 in 
	errno_t err;

	system("D:\\zifujianji\\donghua\\testmax.wmv");	//打开视频或音频文件，
	Sleep(1000);						//窗口等待3秒后输出，可视需求更改时间，单位毫秒

	//循环将.txt文件中的符号打印在控制窗口中，打印完接着打印下一个.txt文件
	//放幻灯片，Sleep()函数调节播放速度
	for (i = 0; i <= 1750; i++)//i 的范围转换为(.txt)文件的个数
	{
		sprintf_s(FileName,
			"D:\\zifujianji\\wengdang1\\(%Id).txt", i);

		err = fopen_s(&in, FileName, "r");
		assert(in != NULL);		//断言结构体指针 in 不为空

		fread(hi, 1, 52800, in);//改 ,这是你一个TXT文件里的字符串. 
